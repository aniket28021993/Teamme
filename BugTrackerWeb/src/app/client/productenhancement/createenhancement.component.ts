...
@Component({
  selector: 'app-createenhancement',
  templateUrl: './createenhancement.component.html',
  styleUrls: ['./createenhancement.component.css']
})
export class CreateenhancementComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.GetAllProductBacklog();
    this.GetAllMapUser();
    this.miscellaneousService.ChangeBreadcrumbData("CREATE ENHANCEMENT");
  }
    ...
    this.enhancementService.CreateEnhancement(Enhancement).subscribe(Enhancement => {
      if (this.FileData != undefined) {
        for (let i = 0; i < this.FileData.length; i++) {
          this.FileUpload(Enhancement, this.FileData[i]);
        }
      }
